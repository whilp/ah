...
- narrow test deps from $(ah_lua) to $(ah_test_deps) — TRYING NOW (iteration 4). define `ah_test_deps` as `$(patsubst %.tl,$(o)/%.lua,$(ah_lib_srcs) $(ah_dep_srcs) $(wildcard sys/tools/*.tl))`. this removes ~32 test .lua compilations and bin/ah.lua from the prerequisite list, so each test only waits for its own .lua + the library/dep/tools files. previous attempt (iteration 3) crashed — likely a variable name or definition issue. this time the variable is defined right after ah_dep_lua and uses the same source variables.
