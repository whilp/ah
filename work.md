...
- narrow test deps from $(ah_lua) to $(ah_test_dep_lua): tests depended on ALL compiled .tl files (including all 32 test files, sys/tools, bin/ah.tl). changed to depend only on lib source .lua files + lib dep .lua files + sys/tools. this lets tests start running as soon as lib+tool sources compile, without waiting for all other test files to compile. each test still depends on its own .lua via the static pattern rule prerequisite. — TRYING NOW (iteration 15, fixed from iteration 14 which missed sys/tools dep)
